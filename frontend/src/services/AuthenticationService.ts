...
import { Dictionary, Duration, ObjectExtensions, Property } from "acts-util-core";
...
{
    constructor(private apiService: APIService, private router: Router, private cookieService: CookieService, apiResponseHandler: APIResponseHandler)
    {
        this._loginInfo = new Property<LoginInfo | undefined>(undefined);

        this.apiService.RegisterInterceptor(new APIServiceInterceptor(this.Logout.bind(this), apiResponseHandler));
    }
    ...
    {
        const sessions = ObjectExtensions.Values(this.QuerySavedSessions()).NotUndefined().Filter(x => !this.IsSessionExpired(x.expiryDateTime)).ToArray();
